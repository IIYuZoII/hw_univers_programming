...
int main() {
  ...
  while (i < count) {
    ...
    sscanf(buff, "%d %s %s %s %s %s %s %s", &(array + i)->is_mail_note,
           (array + i)->msg_theme, (array + i)->msg_text,
  ...
  strcat(specific_sender_mail,
         (array + specific_sender_number)->sender.mail_name);

  //Поиск всех сообщений
  msg_find(array, count, specific_sender, specific_sender_mail,
           msg_from_specific_user);

  //Длинна всех сообщений
  int c = all_msg_lenght(array, count);

  write(fpInput, msg_formatted_line, msg_from_specific_user, specific_sender,
        specific_sender_mail, c);
